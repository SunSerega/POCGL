...
// 3.1.2 - Создание очередей из подпрограммы для CPU
// 
...
// Которая считает сначала то, для чего её вызвали
// А затем применяет к полученному значению какое то преобразование
// 
...
// Или для дебага надо как то по-простому одноразово вызврать 1 команду
// 
...
// Соответствующих методов у очереди - нету
// Методы ".Get..." создают новый объект типа записи или массива,
// читают в него содержимое буфера,
// и возвращают этот полученный объект
// Они так же используют неявную очередь (для чтения буфера)
// 

// 3.1.7 - Buffer.ValueQueue (Очередь из записи)
// 
// Статичный метод Buffer.ValueQueue создаёт новый буфер
// Затем создаёт из него новую очередь
// И добавляет в полученную очередь команду записи значения в эту очередь
// 
// Это полезно, к примеру, если вам надо передать одним из параметров в карнел - размер передаваемого массива, или что то типа того
// Но будьте осторожны, этим методом не стоит злоупотреблять, потому что каждый вызов создаёт новый буфер и новую очередь
// 
...
// 
// Так же, никто не запрещает передавать очередь-параметр, в команду,
// которая сама является частью другой очереди-параметра
...
// Это делается методом CommandQueue.Clone
// 
// Ни 1 из методов, комбинирующих очереди (+,*,ThenConvert,Сycle и т.д.) - не клонируют очереди
// И использовании очереди как параметра - так же не создаёт её копии
// .Clone всегда надо вызывать ручками, если очередь используется в нескольких местах одновременно
// 
// Обычно, если вы напартачили и очередь всё же выполнилась в 2 местах одновременно - вы получите исключение
// Но не надейтесь на это, множественное использование 1 очереди вполне может привести к неопределённому поведению
// И тогда будет нереально понятно что пошло не так
// Лучше следить за этим сразу, при написании кода
// 

{$endregion 3.4 - Клонирование очередей}
...
// Однако память на GPU выделяется только тогда при вызове Buffer.Init
// Исходя из контекста, переданного Buffer.Init - выбирается на каком девайсе будет выделена память
// После выделения памяти - содержимое буфера НЕ отчищается нулями
