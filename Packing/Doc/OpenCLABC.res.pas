...
type
  ...
  __EventList = sealed class
    ...
    public static procedure AttachCallback(ev: cl_event; cb: Event_Callback; cont: __QueueExecContainer);
...
//    public function Multiusable: ()->CommandQueueBase;
  ...
  CommandQueue<T> = abstract class(CommandQueueBase)
    ...
    ///Создаёт очередь, которая выполнит данную
    ///А затем выполнит на CPU функцию f, используя результат данной очереди и контекст, в котором выполнялась очередь
  ...
  __HostQueue<TInp,TRes> = abstract class(CommandQueue<TRes>)
    ...
    begin
      ...
      begin
        ...
        cont.AddErr( cl.SetUserEventStatus(uev, CommandExecutionStatus.COMPLETE) );
      end).Start;
  ...
  CLTask<T> = sealed class(__QueueExecContainer)
    ...
    ///Вызывает исключение, если оно было вызвано при выполнении
  ...
  BufferCommandQueue = sealed class(__GPUCommandContainer<Buffer>)
    ...
    ///len указывает кол-во байт которое надо скопировать
  ...
  Buffer = sealed class(IDisposable)
    ...
    ///Создаёт буфер указанного размера в байтах
    ///Память на GPU выделяется сразу. При этом контекст указывает на каком устройстве надо выделить память
    ...
    ///В отличии от .Init, данный метод НЕ потоко-безопасен.
    ...
    ///Если байт на последний элемент не хватает, они игнорируются
    ///К примеру, если читать буфер на 10 байт как "array of integer" (4 байт на элемент), то в массиве окажется (10 div 4) = 2 элемента
  ...
  KernelCommandQueue = sealed class(__GPUCommandContainer<Kernel>)
    ...
    ///Каждое выполнение KernelCommandQueue q выполняется ровно один раз
  ...
  Context = sealed class
    ...
    ///Запускает данную очередь и все её подочереди
    ///Как только всё запущено: возвращает объект типа CLTask<>, через который можно следить за процессом выполнения
    public function BeginInvoke(q: CommandQueueBase) := new CLTask<object>(q.Cast&<object>(), self);
    ...
    ///Запускает данную очередь и все её подочереди
    ///Затем ожидает окончания выполнения и возвращает полученный результат
    public function SyncInvoke(q: CommandQueueBase) := BeginInvoke(q).GetRes();
  ...
  ProgramCode = sealed class
    ...
    ///И вызывает на полученном массиве статический метод ProgramCode.Deserialize
...
static procedure __EventList.AttachCallback(ev: cl_event; cb: Event_Callback; cont: __QueueExecContainer) :=
cont.AddErr( cl.SetEventCallback(ev, CommandExecutionStatus.COMPLETE, cb, __NativUtils.GCHndAlloc(cb)) );
      ...
      begin
        ...
        if not allow_sync_enq then
          ...
          begin
            ...
            cont.AddErr( cl.SetUserEventStatus(uev,CommandExecutionStatus.COMPLETE) );
            cont.AddErr( cl.ReleaseCommandQueue(lcq) );
          end).Start else
        begin
          ...
          begin
            ...
            Task.Run(()->cont.AddErr( cl.ReleaseCommandQueue(lcq) ));
